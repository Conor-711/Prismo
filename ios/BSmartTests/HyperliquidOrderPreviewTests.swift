...
final class HyperliquidOrderPreviewTests: XCTestCase {
    ...
    func testFeesAndBookReadConcurrentlyWithoutOmittingEitherCheck() async throws {
        let reader = ConcurrentQuoteReader()
        _ = try await HyperliquidOrderPreviewProvider(reader: reader, clock: { F.now }, continuousClock: { F.instant })
            .preview(order: Q.order(), wallet: F.wallet, account: Q.snapshot(), reviewedLeverage: 10, reviewedMarginMode: .cross)
        let peak = await reader.peak
        XCTAssertEqual(peak, 2)
    }

    func testReusedConstructionBookKeepsOriginalExpiryAndIsNotReadTwice() async throws {
        let reader = TradingCheckReaderStub(preview: [try Q.fees()])
        let book = try HyperliquidOrderBook.decode(Q.book(), market: Q.market(), now: F.now)
        let observation = HyperliquidBookObservation(book: book, requestedAt: F.now.addingTimeInterval(-4),
            requestedContinuousAt: F.instant.advanced(by: .seconds(-4)))
        let value = try await HyperliquidOrderPreviewProvider(reader: reader, clock: { F.now }, continuousClock: { F.instant })
            .preview(order: Q.order(), wallet: F.wallet, account: Q.snapshot(), reviewedLeverage: 10,
                     reviewedMarginMode: .cross, observedBook: observation)
        let queries = await reader.queries
        XCTAssertEqual(queries, [.fees(owner: F.wallet.address)])
        XCTAssertEqual(value.expiresAt, F.now.addingTimeInterval(1))
        XCTAssertThrowsError(try value.validate(wallet: F.wallet, now: F.now,
    ...
    func testOnlyExactReadQueriesAndNoImplicitApproval() async throws {
        for builder in [nil, try HyperliquidBuilderFee(address: Q.recipient, tenthsOfBasisPoint: 10)] {
            ...
            let reader = TradingCheckReaderStub(preview: responses)
            ...
            if builder != nil { XCTAssertEqual(calls, expected) }
            else { XCTAssertEqual(calls.count, expected.count); XCTAssertTrue(expected.allSatisfy(calls.contains)) }
    ...
    func testInsufficientApprovalStopsBeforeBookAndNeverRaisesTheFeeCeiling() async throws {
        let reader = TradingCheckReaderStub(preview: [try Q.fees(), Data("9".utf8), try Q.book()])
    ...
    func testMissingMetadataAndOverCapacityOrdersStopBeforeIO() async throws {
        for (market, size) in [(try Q.market(scale: nil), "2.5"), (try Q.market(), "9")] {
            let reader = TradingCheckReaderStub(preview: [])
    ...
    func testAgedBookExpiresOnEitherClockWithoutExtendingItsServerLifetime() async throws {
        let reader = TradingCheckReaderStub(preview: [try Q.fees(), try Q.book(at: F.now.addingTimeInterval(-4))])
    ...
    func testSlowIOCannotRefreshAnOldBookEvenIfTheWallClockFreezes() async throws {
        for (serverAge, latency): (Double, Double) in [(0, 5), (4, 1)] {
            ...
            let reader = TradingCheckReaderStub(preview: [try Q.fees(), try Q.book(at: F.now.addingTimeInterval(-serverAge))]) {
    ...
    func testOrderExpiryAlsoBoundsTheContinuousReviewDeadline() async throws {
        let reader = TradingCheckReaderStub(preview: [try Q.fees(), try Q.book()])
    ...
    func testExpiredAccountCannotBeRevivedByFreshBook() async throws {
        ...
        let calls = await reader.queries; XCTAssertLessThanOrEqual(calls.count, 2)
    }
    func testFailureAndLateCancellationDoNotPublishAPreview() async throws {
        let unavailable = TradingCheckReaderStub(preview: [])
        ...
        let reader = TradingCheckReaderStub(preview: [try Q.fees(), try Q.book()]) { if $0 == 1 { withUnsafeCurrentTask { $0?.cancel() } } }
...
private actor ConcurrentQuoteReader: HyperliquidExecutionReading {
    ...
    func read(_ query: HyperliquidExecutionQuery) async throws -> Data {
        ...
        switch query {
        case .fees: return try HyperliquidQuoteFixture.fees()
        case .book: return try HyperliquidQuoteFixture.book()
