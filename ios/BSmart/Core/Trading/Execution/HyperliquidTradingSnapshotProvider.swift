...
struct HyperliquidTradingSnapshotProvider: Sendable {
    ...
    func snapshot(wallet: DeviceWalletSummary, dex: String, coin: String) async throws -> HyperliquidTradingSnapshot {
        ...
        let dexs = try await read(.dexs)
        let meta = try await read(.metadata(dex: dex))
        ...
        let first = try HyperliquidActiveTradingData.decode(await read(.active(owner: wallet.address, coin: coin)),
                                                          owner: wallet.address, market: market)
        let firstPositions = try HyperliquidTradingPositionState.decode(
            await read(.positions(owner: wallet.address, dex: dex)), market: market, now: clock())
        let last = try HyperliquidActiveTradingData.decode(await read(.active(owner: wallet.address, coin: coin)),
                                                         owner: wallet.address, market: market)
        let lastPositions = try HyperliquidTradingPositionState.decode(
            await read(.positions(owner: wallet.address, dex: dex)), market: market, now: clock())
