...
def main() -> None:
    ...
    end_inclusive = parse_day(args.end) if args.end else dt.datetime.now(dt.timezone.utc).date()
    tweet_dirs = [Path(p).expanduser() for p in args.tweet_dir] or DEFAULT_TWEET_DIRS

    con = sqlite3.connect(args.db)
    ensure_schema(con)

    if args.only.strip():
        tickers = sorted({store_ticker(t) for t in args.only.split(",") if is_candidate_tag(t)})
    else:
        tickers = candidate_tickers(con, tweet_dirs, args.top_n, args.min_count)
    if args.limit > 0:
        tickers = tickers[: args.limit]

    print(
        f"[sv-price] db={os.path.abspath(args.db)} tickers={len(tickers)} "
        f"window={start}..{end_inclusive} top_n={args.top_n} min_count={args.min_count}",
        flush=True,
    )
    ...
    def fetch_one(ticker: str) -> tuple[str, list[tuple], str | None]:
        try:
            rows = fetch_history(ticker, start, end_inclusive)
            if not rows:
