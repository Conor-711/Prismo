# Smart Account Architecture Guide

本文件是 Smart Account 的总入口文档。后续新对话或新 Agent 接手 Smart Account 相关任务时，应先阅读本文件，再按任务读取对应的核心算法和平台 adapter 文件。
...
```text
1. docs/smart_account/README.md
2. SCORE_ALGORITHM.md
3. docs/smart_account/GLOBAL_ALGORITHM.md
4. 当前任务涉及的平台文件：
...
```text
SCORE_ALGORITHM.md
= Smart Account 的共用核心算法：call schema、权重、结算、路径评分、生命周期、聚合、置信度。
...
- `docs/smart_account/youtube_author_pool_audit_2026-07-10.md`
- `docs/smart_account/youtube_top_bottom_report_2026-07-11.md`
- `docs/smart_account/youtube_transcript_sv_migration_2026-07-11.md`

当前全局榜、分平台榜、重点标的专项榜与数据成熟度审计见：

- `docs/smart_account/sv_ranking_report_2026-07-18.md`
- `docs/smart_account/sv_head_tail_evidence_2026-07-18.md`：全局 Top/Bottom 10 的分数拆解、代表性原帖与市场结算证据。
- `docs/smart_account/sv_head_tail_original_links_2026-07-18.md`：以原推文/原视频链接为主体的头尾作者证据索引。

不要把五个平台的完整算法复制成五份。平台文件只写“这个平台如何进入共用核心算法”，共用的市场结算和打分逻辑应保留在核心算法中。

## 2. 产品语义

Smart Account 不是粉丝数榜，也不是内容质量榜。它衡量的是：

```text
在 bSmart 已收集的投资者宇宙中，谁的公开市场观点更值得用户优先阅读。
```
...
- 非行动性内容不进入 Score 结算，例如纯新闻、纯情绪、纯复盘炫耀、无方向宏观评论。
...
Smart Account 的证据单位不是 ticker mention，而是平台内容单元：
...
- 韩语内容里的 ticker 映射。
- 过滤纯社交、纯情绪、纯新闻。
- 如果平台暴露组合/交易动作，只有具备明确美股/ETF 方向含义时才进入 Score。
...
- 不要把相关度分数加入质量或 Score 准入；相关度用于排序和召回，不代表投资判断能力。
- 不要把纯新闻、纯情绪、纯复盘炫耀放进 Score 结算。
...
```text
1. 在 docs/smart_account/ 新增或更新平台算法文件。
2. 明确 source key。
3. 明确内容单元。
4. 明确投资者 id 映射。
5. 明确 ticker 映射和资产范围。
