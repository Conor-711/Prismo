...
```
Trade
├─ Trader: public ID / nickname / avatar → public profile and public trades
...
| trader.id/nickname/avatarURL | public profile service | display/navigation | existing public profile |
...
Four blocks in fixed order: red short $100, red short $500, green long $500,
green long $100. Buttons include direction arrows and accessibility names.
Amounts mean desired USD position value, not margin multiplied by default 5x.
The existing trade engine's size precision can round the actual notional down.
Shortcuts only prefill `LiveMarketOrderDestination` with the exact coin, direction
and notional. They do not execute on opening. Explicit review/confirmation uses
...
Debug regression fixtures still require an explicit launch flag. The user-requested
Demo preview is separately available through the Feed toolbar in every build; it
loads six fictional trades from `trade-feed-demo.json` only after the user taps
Demo. It never silently replaces a failed live request. Demo mode is labelled on
the navigation title, every row and its local public profile. Source opinions are
...
shows the chosen side and USD notional only, with no order executor, signer,
balance writes or live Feed publication. Switching back clears the demo store.
