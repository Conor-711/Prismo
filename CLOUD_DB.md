...
> **bSmart（bsmart.today，本仓库）现以本地 `data/dev.db` 为唯一真源**——含 `gr_*`/`yt_*`/`kol_*` 等云端**没有**的独有层。
> **别在 bSmart 跑 `make cloud-pull` / `make site-cloud`**：会用「只有 Reddit 核心」的云端快照覆盖本地、**抹掉独有层**（『数据消失』的原因）。已在 Makefile 锁死（`cloud-pull` 默认拒绝、`site-cloud`=`make site`）。bSmart 出站只用 `make site`；改数据前先 `make backup-db`。原始 `data/dev.db` 不提交，Railway 数据通过 `make snapshot-db` 生成的压缩快照更新。云端仍供 bSmart 的 **web 后端**（Auth/埋点/收藏，`NEXT_PUBLIC_*`）+ 只读 `tw_*`(X)。
