      ...
      tooltip: {
        ...
        formatter: (raw: any) => {
          ...
          for (const item of evidence) {
            ...
            lines.push(
              `<span style="color:${directionColor(call.direction)}"><b>${directionLabel(call.direction, zh)}</b></span> · ${call.horizon} · SV ${signed(call.contribution)}`,
            );
          }
  ...
  return (
    <section className="border-t border-line pt-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          ...
          <div className={`mt-1 font-mono text-[13px] font-bold ${showcase.focusContribution >= 0 ? "text-bull" : "text-bear"}`}>
            SV {signed(showcase.focusContribution)}
          </div>
        </div>
      </div>
      ...
      <div className="mt-2 flex items-center gap-3 text-[9px] text-neutral-600">
        ...
        <span className="ml-auto">{zh ? "气泡大小 = SV 影响" : "Bubble size = SV impact"}</span>
      </div>
      ...
      <div className="border-t border-line/70 pt-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`ml-auto font-mono text-[10.5px] font-bold ${selected.contribution >= 0 ? "text-bull" : "text-bear"}`}>
            SV {signed(selected.contribution)}
          </span>
        </div>
