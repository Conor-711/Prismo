...
import {
  ...
  type SvInvestor,
  type SvTickerBoard,
} from "@/features/smart-account/svMock";
  ...
  return (
    <Panel className="overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-reddit">Smart Account</div>
      ...
      <div className="flex items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div>
          <h3 className="font-display text-[14px] font-bold text-cream">
            {zh ? `${board.ticker} 最值得参考的 Score 投资者` : `Top Score voices for ${board.ticker}`}
          </h3>
        ...
        <span className="rounded-md bg-reddit/12 px-2 py-1 text-[11px] font-bold text-reddit ring-1 ring-inset ring-reddit/25">Score</span>
      </div>
      <div>
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 font-display text-[15px] font-bold leading-none text-cream">
            {zh ? "作者 Score 画像" : "Creator Score profile"}
          </h2>
...
export function SmartVoicePortfolioModule({
  ...
  return (
    <Panel className="overflow-hidden">
      <div className={`flex items-start justify-between gap-3 border-b border-line ${compact ? "px-3 py-2.5" : "px-4 py-3"}`}>
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-reddit">Smart Account</div>
          <h2 className="mt-1 font-display text-[15px] font-bold leading-none text-cream">
        ...
        <div className="flex max-w-[360px] flex-wrap justify-end gap-1">
          {board.tickers.slice(0, compact ? 4 : 8).map((t) => (
      ...
      <div className={compact ? "grid gap-0" : "grid gap-0 lg:grid-cols-5"}>
        {board.investors.map((inv, i) => (
          <div
            key={inv.id}
            className={
              compact
                ? "border-b border-line/70 p-3 last:border-b-0"
                : "border-b border-line/70 p-3 last:border-b-0 lg:border-b-0 lg:border-r lg:last:border-r-0"
            }
          >
            <div className="mb-2 flex items-center justify-between">
              <span className="font-mono text-[12px] font-bold text-neutral-600">#{i + 1}</span>
              <SmartVoiceScore score={inv.portfolioSv} size="sm" label="Fit Score" />
            </div>
