      ...
      tooltip: {
        ...
        formatter: (raw: any) => {
          ...
          for (const item of evidence) {
            ...
            const positive = (call.contribution ?? 0) >= 0;
            lines.push(
              `<span style="color:${positive ? POSITIVE : NEGATIVE}"><b>${positive ? "+" : "−"} ${zh ? "Score 贡献" : "Score contribution"} ${Math.abs(call.contribution ?? 0).toFixed(2)}</b></span> · ${directionLabel(call.direction, zh)} · ${call.horizon}`,
            );
          }
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`rounded px-1.5 py-px text-[10.5px] font-bold ${positive ? "bg-bull/10 text-bull" : "bg-bear/10 text-bear"}`}>
                {positive ? (zh ? "Score 加分" : "Score positive") : (zh ? "Score 扣分" : "Score negative")} {positive ? "+" : "−"}{Math.abs(selected.contribution ?? 0).toFixed(2)}
              </span>
