...
import { SV_HORIZONS, type SvBoard, type SvDistribution, type SvHorizon, type SvInvestor } from "@/features/smart-account/svMock";
    ...
    <Panel className="flex min-h-0 flex-col overflow-hidden">
      <div className="flex items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div>
          ...
          <h2 className="mt-1 font-display text-[17px] font-extrabold text-cream">{zh ? "市场 Score 分布" : "Market Score distribution"}</h2>
        </div>
        <div className="text-right text-[11px] text-neutral-500">
          <div>{board.scoringVersion ?? "Score"}</div>
...
export function AlertsPanel({ board, zh }: { board: SvBoard; zh: boolean }) {
  const alerts = useMemo(() => {
    ...
    if (withDelta.length) {
      ...
      return [
        svUp && typeof svUp.svDelta === "number" && svUp.svDelta > 0 && {
          tone: "text-bull",
          title: zh ? "Score 上升最快" : "Fastest Score riser",
          body: zh ? `@${svUp.handle} Score ${svUp.sv}，较上次 +${svUp.svDelta}。` : `@${svUp.handle} is at Score ${svUp.sv}, up +${svUp.svDelta}.`,
        },
        svDown && typeof svDown.svDelta === "number" && svDown.svDelta < 0 && {
          tone: "text-bear",
          title: zh ? "Score 下滑最快" : "Fastest Score drop",
          body: zh ? `@${svDown.handle} Score ${svDown.sv}，较上次 ${svDown.svDelta}。` : `@${svDown.handle} is at Score ${svDown.sv}, down ${svDown.svDelta}.`,
        },
    ...
    return [
      top && {
        tone: "text-bull",
        title: zh ? "最高 Score 继续集中" : "Highest Score concentration",
        body: zh ? `@${top.handle} 当前 Score ${top.sv}，排名 #${top.rank ?? 1}。` : `@${top.handle} is at Score ${top.sv}, rank #${top.rank ?? 1}.`,
      },
      highDispersion && {
        tone: "text-amber",
      ...
      weakHighConfidence && {
        ...
        body: zh
          ? `@${weakHighConfidence.handle} 样本充分但 Score ${weakHighConfidence.sv}，需要关注系统性负贡献。`
          : `@${weakHighConfidence.handle} has enough evidence but only Score ${weakHighConfidence.sv}.`,
      },
      short && {
        tone: "text-reddit",
  ...
  return (
    <Panel className="overflow-hidden">
      <div className="border-b border-line px-4 py-3">
        <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-reddit">Score Alert</div>
        <h2 className="mt-1 font-display text-[17px] font-extrabold text-cream">{zh ? "Smart Account 警报" : "Smart Account alerts"}</h2>
      </div>
      ...
      <div className="border-t border-line px-4 py-2 text-[11px] text-neutral-600">
        {hasDelta
          ? (zh ? "已接入历史快照：警报基于最近两次 Score 导出之间的真实变化。" : "Historical snapshots connected: alerts use real changes between the latest two Score exports.")
          : (zh ? "注：首次快照暂无 delta；再次运行 export 后会显示真实 Score 变动。" : "Note: no delta on the first snapshot; run export again after the next score to show true Score changes.")}
      </div>
    </Panel>
  );
}
  ...
  return (
    <Panel className="flex min-h-0 flex-col overflow-hidden">
      <div className="border-b border-line px-4 py-3">
        ...
        <h2 className="mt-1 font-display text-[17px] font-extrabold text-cream">{zh ? "Score 排名监控" : "Score rank monitor"}</h2>
      </div>
      <div className="grid min-h-0 flex-1 grid-cols-2 divide-x divide-line">
        <div className="min-w-0 overflow-y-auto">
          <div className="sticky top-0 z-10 border-b border-line bg-panel/95 px-3 py-2 text-[12px] font-bold text-bull">{zh ? "Top Score" : "Top Score"}</div>
          {top.map((inv) => <InvestorMini key={inv.id} inv={inv} zh={zh} />)}
        </div>
        <div className="min-w-0 overflow-y-auto">
          <div className="sticky top-0 z-10 border-b border-line bg-panel/95 px-3 py-2 text-[12px] font-bold text-bear">{zh ? "低 Score 观察" : "Low Score watch"}</div>
...
export function TypicalPanel({ board, zh }: { board: SvBoard; zh: boolean }) {
  const cases = useMemo(() => {
    ...
    const rows: [string, SvInvestor | undefined, string][] = [
      [zh ? "最佳整体" : "Best overall", best, zh ? "总 Score 排名第一" : "Highest global Score"],
