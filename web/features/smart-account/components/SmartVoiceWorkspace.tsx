...
type WorkspaceView = "market" | "onchain" | "live";
...
export function SmartVoiceWorkspace({
  ...
  const tickerCount = new Set([
    ...
    ...tickerBoards.all["30D"].newCoverage.map((row) => row.ticker),
  ]).size;
  ...
  return (
    <ViewportWorkspace className="flex min-h-0 flex-col overflow-hidden" bottomOffset={16}>
      <header className="flex shrink-0 items-end justify-between gap-5 border-b border-line pb-2.5">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="font-display text-[22px] font-extrabold leading-none text-cream">Smart Account</h1>
            <span
              title={zh ? "根据作者公开观点的历史结算表现、胜率稳定性、覆盖范围与有效样本识别值得参考的声音。Score 不代表作者真实持仓。" : "Identifies valuable public voices from settled call performance, consistency, coverage and effective samples. Score does not represent actual holdings."}
      ...
      <nav className="flex h-11 shrink-0 items-end gap-1 border-b border-line" aria-label={zh ? "Smart Account 视图" : "Smart Account views"}>
        {tabs.map((tab) => (
          <button
            key={tab.key}
            type="button"
        ...
        <div className="ml-auto flex items-center gap-3 pb-1.5">
          <span className="text-[10px] text-neutral-600">
            {zh ? `覆盖 ${stats.platformCount || 4} 个来源 · ${boardMeta.scoringVersion ?? "Score"}` : `${stats.platformCount || 4} sources · ${boardMeta.scoringVersion ?? "Score"}`}
          </span>
          <LocaleLink
            href="/smart-account/leaderboard"
            className="flex h-7 items-center rounded-md px-2.5 text-[10.5px] font-bold text-reddit ring-1 ring-inset ring-reddit/30 transition hover:bg-reddit/10"
          >
            {zh ? "公开投资者榜" : "Public leaderboard"}
