...
function WhyList({ profile, board, zh }: { profile: SvInvestor; board: SvBoard; zh: boolean }) {
  ...
  const points = band === "top"
    ? [
        ...
        topTicker ? (zh ? `代表标的 ${topTicker.key} 的分段 Score 为 ${topTicker.value}。` : `Representative ticker ${topTicker.key} has segment Score ${topTicker.value}.`) : "",
      ]
    : [
        ...
        topNarrative ? (zh ? `主要叙事 ${topNarrative.label?.zh ?? topNarrative.key} 的分段 Score 为 ${topNarrative.value}。` : `Main narrative ${topNarrative.label?.en ?? topNarrative.key} has segment Score ${topNarrative.value}.`) : "",
      ];
  ...
  return (
    <Panel className="p-4 sm:p-5">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-reddit">Score Explanation</div>
          <h2 className="mt-1 font-display text-[18px] font-extrabold text-cream">
            {band === "top" ? (zh ? "为什么 Score 这么高" : "Why this Score is high") : (zh ? "为什么 Score 这么低" : "Why this Score is low")}
          </h2>
    ...
    <Panel className="p-4 sm:p-5">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-1 font-display text-[17px] font-bold text-cream">{zh ? "Score 贡献证据与价格路径" : "Score evidence on price action"}</h2>
