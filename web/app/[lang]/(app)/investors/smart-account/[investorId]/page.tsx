...
import { SmartAccountInvestorProfile } from "@/features/smart-account";
import {
  getSmartVoiceBoard,
  getSmartVoiceDetailInvestors,
  getSmartVoiceInvestor,
} from "@/features/smart-account/svMock";
import { smartVoiceInvestorIdFromSlug, smartVoiceInvestorSlug } from "@/features/smart-account/svInvestorLinks";
import type { SvBoard, SvInvestor, SvPlatformBand } from "@/features/smart-account/svMock";
...
export function generateMetadata({ params }: { params: { lang: string; investorId: string } }): Metadata {
  ...
  return { title: `${profile?.name ?? (zh ? "作者" : "Investor")} · Smart Account · bSmart` };
}
...
export default function SmartAccountInvestorPage({ params }: { params: { lang: string; investorId: string } }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <LocaleLink href="/smart-account" className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-neutral-500 ring-1 ring-inset ring-line transition hover:text-reddit">
          ←
        </LocaleLink>
        <span className="text-[12px] font-medium text-neutral-500">{zh ? "Smart Account / 作者详情" : "Smart Account / Investor detail"}</span>
      </div>
      <SmartAccountInvestorProfile
        profile={profile}
        board={profileBoard}
        evidence={evidence}
