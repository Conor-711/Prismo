...
// 当前为离线生成的真实数据；没有对应 ticker 时模块按需隐藏。
...
export type TopInvestor = {
  ...
  tickerCalls?: number; // 该标的已结算 call 数
  tickerHit?: number; // 该标的命中率 0..1（对照 base 看）
  pltrCalls?: number; // 旧字段名兼容：该标的已结算 call 数
  pltrHit?: number; // 旧字段名兼容：该标的命中率 0..1
