...
const VKEY = "bsmart:vid"; // 持久访客 id（localStorage）
const SKEY = "bsmart:sid"; // 会话 id（sessionStorage）
const DNT = "bsmart:dnt"; // 「不记录本设备」标记（管理员/自己），排除自有访问
const FKEY = "bsmart:fseen"; // 首次访问时间戳 ms（localStorage）→ 新/回访判定 + 同期群
const SSTART = "bsmart:sstart"; // 本会话是否已发过 session_start（sessionStorage）
...
function classifyChannel(host: string | null, utmMedium: string | null, utmSource: string | null): string {
  ...
  if (/(^|\.)bSmart\.xyz$/.test(h)) return "internal";
