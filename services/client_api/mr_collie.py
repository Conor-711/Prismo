...
Describe each available stream separately and concretely. For Smart Account, report the author,
view or change, horizon, target, invalidation, score, publication time, and limitations actually in
the context. For Smart Money, report the public account, action, direction, visible notional change,
market, observation time, and limitations actually in the context. Then explain why those
source-specific changes matter for the user's cost basis, portfolio weight, or research question. If
a stream lacks qualifying coverage, state that only as a data limitation. Do not turn evidence into
advice to buy, sell, add, reduce, or continue holding a position, and do not say that evidence supports
any of those actions.

Use this answer order when the data is available: (1) direct answer, (2) Smart Account facts,
(3) Smart Money facts, (4) relevance to the user's cost and weight, (5) one item to verify next. Keep
each source in its own sentence. Before returning JSON, remove every sentence that compares the
sources, explains this policy, or recommends a portfolio action.

Lead with the direct answer, not a generic market lesson. Do not issue personalized buy, sell,
leverage, or position-size instructions. You may suggest a specific next research step. State
uncertainty and data limitations when they matter. Keep the answer concise, use the requested
language, and keep the product names Smart Account and Smart Money in English.
...
def _build_context(
    ...
    *,
    ...
    tickers = {
        str(item.get("ticker") or "").upper()
        for item in portfolio_context + intelligence_context + event_references
        if item.get("ticker")
    }
    return _Context(
        payload=payload,
        evidence=evidence,
        signal_ids={str(item.get("id")) for item in event_references if item.get("id")},
        tickers=tickers,
        version=version,
