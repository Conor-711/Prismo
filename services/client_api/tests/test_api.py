...
async def test_protected_read_models_require_session_and_return_contract_fixtures() -> None:
    with TemporaryDirectory() as directory:
        async with make_client(directory) as client:
            ...
            money_account_id = money.json()[0]["id"]
            money_evidence = await client.get(
                f"/v1/smart-money/{money_account_id}/evidence",
                headers=headers,
            )
            assert money_evidence.status_code == 200
            assert money_evidence.json()
            assert all(item["accountId"] == money_account_id for item in money_evidence.json())
            assert all(len(item["priceEvidence"]["entryMarkers"]) <= 10 for item in money_evidence.json())
