...
async def _client(directory: str, answer: dict):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert "independent, parallel evidence streams" in system_prompt
        assert "Never compare the streams" in system_prompt
        assert "Do not explain or disclaim this policy" in system_prompt
        assert "evidence_catalog" in body["messages"][1]["content"]
        prompt = json.loads(body["messages"][1]["content"])
        context = prompt["context"]
        assert "signals" not in context
        assert context["event_references"]
        assert all(
            not ({"kind", "direction", "conclusion", "position_impact", "next_step", "limitations"} & set(item))
        ...
        assert all(
            not ({"relationship", "direction", "conclusion"} & set(item))
